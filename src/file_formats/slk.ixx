	...
	export class SLK {
	  ...
	  public:
		...
		void load(const fs::path& path, const bool local = false) {
			const auto buffer = [&] {
				if (local) {
					...
					if (!res) {
						throw std::runtime_error("Failed to load local SLK " + path.string() + ": " + res.error());
					}
					return std::move(res.value().buffer);
				} else {
					auto res = hierarchy.open_file(path);
					if (!res) {
						throw std::runtime_error("Failed to load SLK " + path.string() + ": " + res.error());
					}
					return std::move(res.value().buffer);
				}
			}();
		...
		T data(const std::string_view column_header, const std::string_view row_header) const {
			...
			if (!data) {
				const auto fallback_columns = hierarchy.resource_priority == Hierarchy::ResourcePriority::ReforgedFirst
					? std::array{std::string(column_header) + ":hd", std::string(column_header) + ":sd"}
					: std::array{std::string(column_header) + ":sd", std::string(column_header) + ":hd"};

				for (const auto& fallback_column : fallback_columns) {
					data = data_single_asset_type(fallback_column, row_header);
					if (data) {
						break;
					}
				}
