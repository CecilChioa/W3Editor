...
namespace {
	std::string mpq_path_string(const fs::path& path) {
	...
	export class MPQ {
	  public:
		...
		File file_open(const fs::path& path) const {
			File file;
			const std::string mpq_path = mpq_path_string(path);
			const bool opened = SFileOpenFileEx(handle, mpq_path.c_str(), 0, &file.handle);
			if (!opened) {
				throw std::runtime_error("Failed to read file " + mpq_path + " with error: " + std::to_string(GetLastError()));
			}
			return file;
		}
		...
		bool file_exists(const fs::path& path) const {
			const std::string mpq_path = mpq_path_string(path);
			return SFileHasFile(handle, mpq_path.c_str());
		}
