...
W3Editor::W3Editor(QWidget* parent)
	: QMainWindow(parent) {
	...
	connect(ui.ribbon->import_heightmap, &QPushButton::clicked, this, &W3Editor::import_heightmap);
	...
	// connect(ui.ribbon->new_map, &QAction::triggered, this, &W3Editor::load);
	connect(ui.ribbon->open_map_folder, &QPushButton::clicked, this, &W3Editor::load_folder);
	connect(ui.ribbon->open_map_mpq, &QPushButton::clicked, this, &W3Editor::load_mpq);
	connect(ui.ribbon->save_map, &QPushButton::clicked, this, &W3Editor::save);
	connect(ui.ribbon->save_map_as, &QPushButton::clicked, this, &W3Editor::save_as);
	connect(ui.ribbon->export_mpq, &QPushButton::clicked, this, &W3Editor::export_mpq);
	connect(ui.ribbon->test_map, &QPushButton::clicked, this, &W3Editor::play_test);
	connect(ui.ribbon->settings, &QPushButton::clicked, [&]() { new SettingsEditor(this); });
	connect(ui.ribbon->switch_warcraft, &QPushButton::clicked, this, &W3Editor::switch_warcraft);
	...
	connect(ui.ribbon->trigger_editor, &QRibbonButton::clicked, [this]() {
		...
		connect(this, &W3Editor::saving_initiated, editor, &TriggerEditor::save_changes, Qt::UniqueConnection);
	});
...
void W3Editor::load_folder() {
	...
	map->render_manager.resize_framebuffers(ui.widget->width(), ui.widget->height());
	setWindowTitle("W3Editor 0.10 - " + QString::fromStdString(map->filesystem_path.string()));
}

/// Load MPQ will extract all files from the archive in a user specified location
void W3Editor::load_mpq() {
	QSettings settings;
...
void W3Editor::closeEvent(QCloseEvent* event) {
...
void W3Editor::resizeEvent(QResizeEvent* event) {
	QMainWindow::resizeEvent(event);
	QTimer::singleShot(0, [&] { save_window_state(); });
}

void W3Editor::moveEvent(QMoveEvent* event) {
	QMainWindow::moveEvent(event);
	QTimer::singleShot(0, [&] { save_window_state(); });
}

void W3Editor::switch_warcraft() {
	QSettings settings;
	fs::path directory;
	do {
...
void W3Editor::import_heightmap() {
...
void W3Editor::save_window_state() {
	QSettings settings;
...
void W3Editor::restore_window_state() {
	QSettings settings;
...
void W3Editor::set_current_custom_tab(QRibbonTab* tab, QString name) {
...
void W3Editor::remove_custom_tab() {
